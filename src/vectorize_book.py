...
vector_db_dir = f"{parent_dir}/vectors_db"
chapters_vector_db_dir=f"{parent_dir}/chapters_vectors_db"

embeddings= HuggingFaceEmbeddings()
text_splitter = CharacterTextSplitter(chunk_size=2000,chunk_overlap=500)

def vectorize_book_and_store_in_db(class_subject_name,vector_db_name):
    book_dir=f"{data_dir}/{class_subject_name}"
    vector_db_path=f"{parent_dir}/{vector_db_name}"
    loader= DirectoryLoader(path=book_dir,glob="./*.pdf",loader_cls=UnstructuredFileLoader)
    documents= loader.load()
    text_chunks= text_splitter.split_documents(documents)
    Chroma.from_documents(documents =text_chunks,embedding=embeddings,persist_directory=vector_db_path)
...
def vectorize_chapters(class_subject_name):
    ...
    for chapter in os.listdir(book_dir):
        ...
        Chroma.from_documents(documents =texts,embedding=embeddings,persist_directory=f"{chapters_vector_db_dir}/{chapter_name}")
